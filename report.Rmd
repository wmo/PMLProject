...
- check the given data, see which columns make most chance of providing a correct response, and generate a model fit. 

- run this model on the training data and testing data.  

- the chosen method is "randomForest", because it is one of the best performing algorithms

- Given datasets, two files:
...
Here is the overview about how the data from the given files is reorganized into dataframes and vectors:

![](data_objects.png)

Note: the splitting into training and test data occurs randomly, and not at a fixed index into the data. 
...
A number of columns where manually selected to be excluded (eg. "raw_timestamp_part_1" because of timestamp data)
...
ytrain_v=y_v[train]
dim(xtrain_df); length(ytrain_v)
```

```{r}
xtest_df=x_df[-train,] 
ytest_v=y_v[-train]
dim(xtest_df); length(ytest_v)
```
...
Make a prediction on the training data, to see how correct the data is. 
...
This is the data submitted to the coursera website for judging. And it turned out to be 100% correct.
